...
/**
 * Pure synchronous SHA-256 over the UTF-8 encoding of `input`.
 *
 * Hand-rolled instead of `crypto.createHash` so the codec stays importable
 * from a browser/webview context (the desktop companion vendors this file).
 * The digest feeds persisted deduplication tags, so the UTF-8 encoder must
 * match Node's byte for byte - including the U+FFFD substitution Node applies
 * to unpaired surrogates.
 */
function sha256Hex(input: string): string {
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (charCode < 0x80) {
            ...
            const low = charCode < 0xdc00 && i + 1 < input.length ? input.charCodeAt(i + 1) : 0;
            if (low >= 0xdc00 && low <= 0xdfff) {
                ...
                // Unpaired surrogate: encode U+FFFD, exactly as Node does.
...
export function sourceCommentImportTags(
    ...
    return records.map((record) => {
        ...
        const digest = sha256Hex(payload).slice(0, 16);
...
export function sourceCommentAnnotationIdFingerprint(annotationId: string): string {
    return sha256Hex(annotationId).slice(0, 32);
}
