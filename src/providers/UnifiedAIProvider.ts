...
export class UnifiedAIProvider {
            ...
            if (!apiKey) {
                ...
                if (!secretKey) {
                    ...
                    switch (this.config.provider) {
                        ...
                        case 'ollama':
                            helpMessage =
                                'Ollama should work without an API key. Make sure Ollama is running locally on port 11434';
                            break;
                    }
    ...
    private buildPrompt(
        code: string,
        filePath: string,
        context?: { language?: string; projectInfo?: string; additionalContext?: string }
    ): string {
    ...
    private async callAI(prompt: string): Promise<string> {
        ...
        try {
            ...
            if (!this.models.chat || !Array.isArray(this.models.chat) || this.models.chat.length === 0) {
                throw new Error(
                    `No chat models available for ${this.config.provider}. Please check your API key and provider settings.`
                );
            }
            ...
            if (typedError.message?.includes('401') || typedError.message?.includes('Unauthorized')) {
                ...
                throw new Error(
                    `Cannot connect to ${this.config.provider}. Please check if the service is running (especially for Ollama).`
                );
            }
